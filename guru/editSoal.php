...
<body>
    ...
    <?php 
        ...
        $queryTugas = mysqli_query($conn, "SELECT * FROM tbl_tugas INNER JOIN tbl_guru ON tbl_guru.id_user = tbl_tugas.id_guru WHERE tbl_tugas.materi = '$materi' AND tbl_tugas.id_guru = '$idUser' AND tbl_tugas.kelas = '$kelas2' AND tbl_tugas.id_tugas = '$idTugas' AND jenis_soal = 'Pilihan Ganda'");
        ...
        if( isset($_POST['buat']) ) {
            ...
            while( $no <= $jumlahSoal ) {
                ...
                $soal = $_POST['soal' . $no];
                $pg = $_POST['pg' . $no];
                $id_soal = $_POST['id_soal' . $no];

                // $querySoal = mysqli_query($conn, "INSERT INTO tbl_soal VALUES('', '$idTugas', '$no', '$soal', '$pg')");

                $querySoal = mysqli_query($conn, "UPDATE tbl_soal SET soal = '$soal', jawaban_benar = '$pg' WHERE id_soal = '$id_soal'");
                ...
                mysqli_query($conn, "UPDATE tbl_jawaban SET desk = '$jawabanA' WHERE id_soal = '$id_soal' AND pg = 'A'");
                mysqli_query($conn, "UPDATE tbl_jawaban SET desk = '$jawabanB' WHERE id_soal = '$id_soal' AND pg = 'B'");
                mysqli_query($conn, "UPDATE tbl_jawaban SET desk = '$jawabanC' WHERE id_soal = '$id_soal' AND pg = 'C'");
                mysqli_query($conn, "UPDATE tbl_jawaban SET desk = '$jawabanD' WHERE id_soal = '$id_soal' AND pg = 'D'");
            ...
            <div class="col-md-12">
                <div class="card card2 card3">
                    <div class="detail d-flex w-100 align-items-center">
                        <a href="editTugas.php?idTugas=<?= $idTugas ?>&kelas=<?= $kelas2 ?>">
                            <div class="bulat">
                                <i class="fas fa-arrow-left"></i>
                            </div>
                        </a>
                        <h2 class="text-center mx-auto tT">Edit soal</h2>
                    </div>
                    <form action="" method="POST">
                        ...
                        <div class="soal mt-5 ml-4">
                            <div class="detail-soal d-flex" style="width: 100% !important;">
                                ...
                                <div class="form-group ml-3" style="width: 50%;">
                                    <textarea name="soal<?= $i; ?>" class="form-control input2"
                                        id="exampleFormControlTextarea1" rows="3" required><?= $t['soal'] ?></textarea>
                                </div>
                            </div>
                            <?php 
                            ...
                            <div class="jawaban ml-3">
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="radio" name="pg<?= $i; ?>" id="pg1"
                        ...
                        <?php endwhile; ?>
                        <?php endwhile; ?>
                        <button type="submit" class="btn btn-kerjakan w-100 mt-5" name="buat">Edit Soal</button>
                    </form>
                </div>
            </div>
        </div>
