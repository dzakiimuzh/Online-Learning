<?php 
    ...
    $queryTugas = mysqli_query($conn, "SELECT * FROM tbl_tugas INNER JOIN tbl_guru ON tbl_guru.id_user = tbl_tugas.id_guru WHERE tbl_tugas.materi = '$materi' AND tbl_tugas.id_guru = '$idUser' AND tbl_tugas.kelas = '$kelas2' AND tbl_tugas.status = 'belum' AND jenis_soal = 'Essay'");

    $qTugas = mysqli_query($conn, "SELECT * FROM tbl_soal_essay WHERE id_tugas = '$idTugas'");
    $qTugas2 = mysqli_query($conn, "SELECT * FROM tbl_soal_essay WHERE id_tugas = '$idTugas'");
    ...
    if( isset($_POST['edit']) ) {
        ...
        while( $no <= $jumlahSoal ) {
            ...
            $row2 = mysqli_fetch_assoc($qTugas2);
            $idEssay = $row2['id_essay'];

            $soal = $_POST['soal' . $no];

            $querySoal = mysqli_query($conn, "UPDATE tbl_soal_essay SET soal = '$soal' WHERE id_essay = '$idEssay'");
        ...
        if( $querySoal ) {
            ...
            echo "
                ...
                <script>
                    ...
                    document.location.href = 'editSoalEssay.php?materi=$materi&kelas=$kelas2&idTugas=$idTugas';
                </script>
            ...
            <div class="col-md-12">
                <div class="card card2 card3">
                    ...
                    <form action="" method="POST">
                        ...
                        <?php $s = mysqli_fetch_assoc($qTugas); ?>
                            ...
                            <div class="detail-soal d-flex" style="width: 100% !important;">
                                ...
                                <div class="form-group ml-3" style="width: 50%;">
                                    <textarea name="soal<?= $i; ?>" class="form-control input2"
                                        id="exampleFormControlTextarea1" rows="3" required><?= $s['soal'] ?></textarea>
                                </div>
                            </div>
                        </div>
                        <?php $i++; ?>
                        <?php endwhile; ?>
                        <button type="submit" class="btn btn-kerjakan w-100 mt-5" name="edit">Edit Soal</button>
                    </form>
                </div>
            </div>
        </div>
