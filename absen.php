<?php 
    ...
    $jumlah_data = count(query("SELECT * FROM tbl_absen WHERE id_user = '$idUser'"));
    $total_halaman = ceil($jumlah_data / $batas);

    $nomor = $halaman_awal+1;

    $qAbsen = mysqli_query($conn, "SELECT * FROM tbl_absen WHERE id_user = '$idUser' ORDER BY id_absen DESC LIMIT $halaman_awal, $batas");

    if( isset($_POST['cari']) ) {

        $keyword = $_POST['keyword'];

        $jumlah_data = count(query("SELECT * FROM tbl_absen WHERE hari LIKE '%$keyword%' AND id_user = '$idUser' OR tanggal LIKE '%$keyword%' AND id_user = '$idUser'"));
        $total_halaman = ceil($jumlah_data / $batas);

        $qAbsen = mysqli_query($conn, "SELECT * FROM tbl_absen WHERE hari LIKE '%$keyword%' AND id_user = '$idUser' OR tanggal LIKE '%$keyword%' AND id_user = '$idUser' ORDER BY id_absen DESC LIMIT $halaman_awal, $batas");
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="nav-row">
                    ...
                    <a href="#" class="btn active btn-nav"><i class="far fa-clipboard mr-2"></i>Absen Saya</a>
                ...
                <div class="card card2">
                        ...
                        <form action="" method="POST">
                            <div class="input-group mb-3">
                                ...
                                <div class="input-group-append">
                                    <button class="btn btn-outline-secondary btnC" type="submit"
                                        id="button-addon2" name="cari">Search</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="over">
                        <table class="table">
                            ...
                            <tbody>
                                <?php $no = 1; ?>
                                <?php while( $a = mysqli_fetch_assoc($qAbsen) ) : ?>
                                <?php 
